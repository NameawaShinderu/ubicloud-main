...
class Hosting::Apis
  def self.pull_ips(vm_host)
    case vm_host.provider_name
    when HostProvider::HETZNER_PROVIDER_NAME
      vm_host.provider.api.pull_ips
    when HostProvider::ONPREM_PROVIDER_NAME
      vm_host.provider.api.pull_ips
    else
  ...
  # Add new method for on-premises specific operations
  def self.check_provider_connectivity(vm_host)
    case vm_host.provider_name
    when HostProvider::ONPREM_PROVIDER_NAME
      vm_host.provider.api.check_connectivity
    else
      # For other providers, assume connectivity is managed externally
      true
    end
  end

  def self.get_system_info(vm_host)
    case vm_host.provider_name
    when HostProvider::ONPREM_PROVIDER_NAME
      vm_host.provider.api.get_system_info
    else
      {}
    end
  end
